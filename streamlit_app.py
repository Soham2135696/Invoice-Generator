...
genai.configure(api_key="AIzaSyAaD4wmF-QsHtw2f0owFl2oUT842A-dDB8") #config.
...
st.sidebar.write("Made by SOHAM")
st.sidebar.write("Powered by google gemini ai")
st.header("Robobill")
st.subheader("Made by SOHAM")
st.subheader("Manage your expenses with Robobill")
input = st.text_input("What do you want me to do?",key="ínput")
uploaded_file = st.file_uploader("Choose an image",type=['jpg','jpeg','png'])
image = ""
if uploaded_file is not None:
    image = Image.open(uploaded_file)
    st.image(image,caption="Uploaded Image",use_column_width=True)
ssubmit = st.button("Chaliye Shuru Karte Hain!")
